...
### Flyweight (Легковес, Кэш) - пример структурного паттерна
https://refactoring.guru/ru/design-patterns/flyweight  
`Цель` - позволяет вместить бóльшее количество объектов в отведённую оперативную память.
...
Паттерн разделяет данные этих объектов на две части — легковесы (внутреннее состояние) и контексты (внешнее состояние).

`Примеры использования`:
- Отрисовка большого количества одинаковых объектов в игре, например деревьев.
- Легковес - тип дерева, который может содержать цвет, название (на него в объекте будет дана ссылка)
- Контекст - координаты дерева, полотно отрисовки
- Можно создать фабрику, которая будет порождать деревья
...
---
Код:  
https://gist.github.com/VladislavSoren/b955e167237b75a3e913fa3d4d84356c


### Proxy (Заместитель) - пример структурного паттерна
https://refactoring.guru/ru/design-patterns/proxy  
`Цель` - позволяет подставлять вместо реальных объектов специальные объекты-заменители. 
...
Существует два подвида этого отношения: 
если один объект создает другой объект и время жизни "части" зависит от времени жизни целого, то это называется "композиция", 
если же один объект получает ссылку (указатель) на другой объект в процессе конструирования, то это уже агрегация.

Композиция («сильная ассоциация») - объект (подкласс) порождается в родителе по переданным параметрам и умирает вместе с ним
```
class Car:
    def __init__(self, size, power):
        self.engine = Enginne(size, power)
        
car = Car(5, 100)
```

Агрегация («слабая ассоциация») - класс содержит ссылку (указатель) на объект, но объект существует и вне родителя
```
class Order:
